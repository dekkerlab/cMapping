...
sub check_options {
    ...
    my ($cDataDirectory,$readDir,$outputDir,$genomeName,$projectName,$hicMode,$fiveCMode,$experimentPrefix);
...
sub findDataFiles($$$$$$$) {
    ...
    for my $eachFile (readdir($dir)) {
        ...
        my $file = $parentDir .$eachFile;
        if( -d $file) {
            &findDataFiles($laneData,$mappingData,$cDataDirectory,$file,$genomeName,$hicMode,$fiveCMode);
        } else {
...
my %options;
my $results = GetOptions( \%options,'cDataDirectory|i=s','readDir|r=s','outputDir|o=s','genomeName|g=s','projectName|pn=s','hicMode|h','fiveCMode|f','experimentPrefix|ep=s');

my ($cDataDirectory,$readDir,$outputDir,$genomeName,$projectName,$hicMode,$fiveCMode,$experimentPrefix)=check_options( \%options );

print "\n";
print "cDataDirectory (-i)\t$cDataDirectory\n";
...
my $laneData={};
($laneData,$mappingData)=findDataFiles($laneData,$mappingData,$cDataDirectory,$cDataDirectory,$genomeName,$hicMode,$fiveCMode);
